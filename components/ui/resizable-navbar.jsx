...
export const NavItems = ({ items, className, onItemClick }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative hidden flex-1 flex-row items-center justify-start space-x-6 text-sm font-bold font-nav tracking-wide text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex",
        className,
      )}
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-neutral-600 font-nav font-bold uppercase text-lg hover:text-primary transition-colors dark:text-neutral-300"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
